...
/**
 * Pull Reading Calendar events for a given week
 * @param {number} weekNumber - Week number (1-52)
 * @returns {Object} - Object with "Reading Events" key containing formatted string
 */
async function pullReadingCalendar(weekNumber) {
  try {
    console.log(
      `📥 Fetching Reading Calendar events for Week ${weekNumber}...`
    );

    const { startDate, endDate } = await getWeekDateRange(weekNumber);

    if (!process.env.READ_CALENDAR_ID) {
      console.log("   ⚠️  READ_CALENDAR_ID not configured");
      return {
        "Reading Events":
    ...
    const formattedReading = formatEventsColumn(
      readingEvents,
      "Reading Events",
      "events",
      true
    ...
    return {
      "Reading Events": formattedReading,
    };
  } catch (error) {
    console.error(
      `❌ Error pulling reading calendar for Week ${weekNumber}:`,
      error.message
    );
    return {
      "Reading Events":
        "Reading Events (0 events, 0 hours):\nError fetching reading events this week",
    };
  }
}
